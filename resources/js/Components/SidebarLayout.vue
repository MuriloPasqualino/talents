...
defineProps({
    /** Classes for the outer shell (defeito: gradiente + app-shell em app.css) */
    shellClass: {
        type: String,
        default: 'app-shell',
    },
...
<template>
    <div :class="shellClass">
        ...
        <div
            v-show="mobileOpen"
            class="fixed inset-0 z-40 bg-slate-900/25 backdrop-blur-[2px] lg:hidden"
            aria-hidden="true"
        ...
        <aside
            :class="[
                asideWidthClass,
                asideTransformClass,
                'fixed inset-y-0 left-0 z-50 flex w-64 flex-col border-r border-slate-200/80 bg-white shadow-sm ring-1 ring-slate-200/50 transition-all duration-300 ease-in-out lg:overflow-visible',
            ]"
        ...
        >
            <div class="flex h-full min-h-0 flex-col lg:overflow-visible">
                ...
                <div
                    class="flex min-h-[3.25rem] shrink-0 items-center border-b border-slate-200/80 px-2 py-2"
                ...
                <div class="shrink-0 overflow-visible border-t border-slate-200/80 p-2">
            ...
            <button
                type="button"
                class="absolute right-2 top-3 rounded-md p-1 text-slate-500 hover:bg-slate-100 hover:text-slate-800 lg:hidden"
                @click="closeMobileSidebar"
            >
        ...
        <div :class="['transition-[padding] duration-300 ease-in-out', mainPaddingClass]">
            <!-- Mobile top bar -->
            <div
                class="sticky top-0 z-30 flex h-14 items-center gap-3 border-b border-slate-200/80 bg-white/95 px-4 backdrop-blur-sm lg:hidden"
            >
                <button
                    type="button"
                    class="inline-flex items-center justify-center rounded-lg p-2 text-slate-700 transition hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-talents-500/40"
